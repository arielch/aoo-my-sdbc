...
MYSQLCONNC_INSTALLDIR=$(OUT_COMP_GEN)/$(MYSQL_CONNECTOR_C_VER)
#MYSQLCONNC_INSTALLDIR=$(MYSQLCONNC_DIR)/install_$(MYSQLCONNC)
MYSQLCONNC_LIB=libmysql.$(SHAREDLIB_EXT)
MYSQLCONNC_INC_DIR=$(OUT_COMP_INC)/$(MYSQLCONNC)
MYSQLCONNC_INC=-I$(MYSQLCONNC_INC_DIR)

#ifeq "$(OS)" "WIN"
#MYSQLCONNC_LICENSES = $(foreach license,$(MYSQL_CONNECTOR_C_LICENSE),$(subst /,$(PS),$(OUT_COMP_GEN)/$(MYSQL_CONNECTOR_C_VER)/$(license)))
#else
MYSQLCONNC_LICENSES = $(foreach license,$(MYSQL_CONNECTOR_C_LICENSE),$(subst /,$(PS),$(MYSQLCONNC_INSTALLDIR)/$(license)))
#endif

MYSQLCONNC_PATCH=$(PATCHDIR)/$(MYSQL_CONNECTOR_C_VER).patch
...
ifeq "$(OS)" "WIN"
	...
	CMAKEBUILDCMD=$(MAKE) $(MAKEFLAGS) libmysql
...
#ifeq "$(STLPORT)" "yes"
#MYSQLCONNCPP_CFLAGS += -DADAPT_EXT_STL
#endif
...
$(MYSQLCONNC_PATCHED_FLAG) : $(MYSQLCONNC_PATCH) $(MYSQLCONNC_UNPACKED_FLAG)
	cd $(subst /,$(PS),$(@D)/$(MYSQL_CONNECTOR_C_VER)) && $(PATCH) -p2 -i "$(CURDIR)/$<"
	@echo flagged > $(subst /,$(PS),$@)

$(MYSQLCONNC_CONFIGURED_FLAG) : $(MYSQLCONNC_PATCHED_FLAG)
	cd $(subst /,$(PS),$(@D)/$(MYSQL_CONNECTOR_C_VER)) && $(CMAKECMD)
	@echo flagged > $(subst /,$(PS),$@)

$(MYSQLCONNC_BUILD_FLAG) : $(MYSQLCONNC_CONFIGURED_FLAG)
	cd $(subst /,$(PS),$(@D)/$(MYSQL_CONNECTOR_C_VER)) && $(CMAKEBUILDCMD)
	@echo flagged > $(subst /,$(PS),$@)
